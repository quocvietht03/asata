...
class WPBakeryShortCode_bt_portfolio_grid extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		$css_class = array(
			...
			$rand_class,
			'bt-element bt-portfolio-grid-element bt-iso-grid',
			$layout_mode,
			$layout,
		...
		$wp_query = vc_render_query_group($atts, 'fw-portfolio', 'fw-portfolio-category');
					...
					if($show_filter){
						...
						if (isset($category) && $category != '') {
							...
							foreach ($cat_arr as $cat){
								$term = get_term_by('slug', $cat, 'fw-portfolio-category');
							...
							$terms = get_terms('fw-portfolio-category', 'hide_empty=true');
...
vc_map(array(
	'name' => esc_html__('Portfolio Grid', 'asata'),
	...
	'params' => array_merge(
		...
		vc_map_add_data_group('fw-portfolio-category', 'Data'),
		array(
			array(
				'type' => 'bt_layout',
				'folder' => 'portfolio',
			...
			array(
				...
				'value' => array(
					...
					esc_html__('Layout 1', 'asata') => 'packery-layout1'
				),
				'dependency' => array(
					'element'=>'layout_mode',
					'value'=> 'packery'
