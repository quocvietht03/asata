...
$post_title = isset($asata_options['single_team_title']) ? $asata_options['single_team_title']: false;
...
if(!empty($social)){
	foreach($social as $item){
		$social_item[] = '<li><a href="'.esc_url($item['link']).'" data-btIcon="'.esc_attr($item['icon']).'" data-toggle="tooltip" title="'.esc_attr($item['title']).'"><i class="'.esc_attr($item['icon']).'"></i></a></li>';
	}
}
?>
<article <?php post_class(); ?>>
	<div class="row">
		...
		<div class="col-md-6">
			...
			<div class="bt-info">
				<h4><?php esc_html_e('Information', 'asata'); ?></h4>
				<ul>
			...
			<div class="bt-social">
				<h4><?php esc_html_e('Social', 'asata'); ?></h4>
